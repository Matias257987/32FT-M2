...
createdBy.innerHTML += " Matias Acosta";
...
function buildToDo(todo, index) {
  // Tu código acá:
  const toDoShell = document.createElement("div"); // Agrega un nuevo "DIV".
  toDoShell.className = "toDoShell"; // Agrega una clase al nuevo DIV creado en la linea anterior.

  const toDoText = document.createElement("span"); // Agrega un nuevo "SPAN".

  toDoText.innerHTML = todo.description;
  toDoText.setAttribute("id", index);

  if (this.complete) toDoText.className = "completeText";
...
function buildToDos(toDos) {
  // Tu código acá:
  const misToDos = toDos.map (function (todo, i) {
...
function displayToDos() {
  ...
  buildToDos(toDoItems).forEach(function (toDoShell) {
